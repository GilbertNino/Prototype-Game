var Snake = (function () {
    ...
    RandomFruit: function(){
        if(walls){
            fruit.x = 2+Math.floor(Math.random() * (tileCount-2));
            fruit.y = 2+Math.floor(Math.random() * (tileCount-2));
        }
        else {
    ...
    log:function (){
        ...
        console.log('tail' + tail + ', trail.lenght:' + trail.length)
    },
    ...
    loop: function (){
        ...
        if (velocity.x == 0 && velocity.y == -1) lastAction = ActionEnum.left;
        if (velocity.x == 0 && velocity.y == -1) lastAction = ActionEnum.right;
        ...
        for(var i=0; i<trail.length-1; i++) {
            ...
            //consol.debug(i + ' => player:' + player.x, player.y + ', trail:' + trail[i].x, trail[i].y);
            if (stopped && trail[i].x == player.x && trail[i].y == player.y){

            }
            ctx.fillStyle = 'lime';
        }
        ctx.fillRect(trail[trail.length-1].x * gridSize+1, trail[trail.length-1].y 
            * gridSize+1, gridSize-2, gridSize);
    ...
    if(stopped) {
        ...
        ctx.font = "bold small-caps bold 14px Helvetica";
...
return {
    start: function (fps = 15) {
        window.onload = setup;
        interval1ID = setInterval(game.loop, 1000/ fps);
    }
    loop: game. loop,
